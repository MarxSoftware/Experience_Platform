...
public class ReferrerParser {
	...
	private static final String REFERERS_YAML_PATH = "referrers.yml";
	private Map<String, ReferrerLookup> referers;
	 ...
	 */
	public ReferrerParser() throws IOException {
		this(ReferrerParser.class.getResourceAsStream(REFERERS_YAML_PATH));
	}
	 ...
	 *
	 */
	public ReferrerParser(InputStream referersStream) {
		referers = loadReferers(referersStream);
	}

	public Referrer parse(String refererUri, URI pageUri) throws URISyntaxException {
		return parse(refererUri, pageUri.getHost());
	}

	public Referrer parse(String refererUri, String pageHost) throws URISyntaxException {
	...
	public Referrer parse(URI refererUri, String pageHost) {
		return parse(refererUri, pageHost, Collections.<String>emptyList());
	}

	public Referrer parse(URI refererUri, String pageHost, List<String> internalDomains) {
		if (refererUri == null) {
			return null;
		}
	...
	public Referrer parse(URL refererUrl, String pageHost) {
		if (refererUrl == null) {
			return null;
		}
	...
	private Referrer parse(String scheme, String host, String path, String query, String pageHost) {
		return parse(scheme, host, path, query, pageHost, Collections.<String>emptyList());
	}

	private Referrer parse(String scheme, String host, String path, String query, String pageHost, List<String> internalDomains) {
		...
		ReferrerLookup referer = lookupReferer(host, path, true);
		if (referer == null) {
			referer = lookupReferer(host, path, false);
		}

		Referrer returnReferer;
		if (referer == null) {
			returnReferer = Referrer.UNKNOWN; // Unknown referer, nothing more to do
		} else {
			returnReferer = new Referrer(referer.medium, referer.source);
		}

		Optional<Utm> utm = parseUtmParameters(query);
		if (utm.isPresent()) {
			returnReferer.setUtm(utm.get());
		}
		return returnReferer;
	}

	private Optional<Utm> parseUtmParameters(String query) {
		if (query != null) {
			Map<String, List<String>> paramsMap = splitQuery(query);
			String source = paramsMap.containsKey("utm_source") ? paramsMap.get("utm_source").get(0) : "";
			String medium = paramsMap.containsKey("utm_medium") ? paramsMap.get("utm_medium").get(0) : "";
			String term = paramsMap.containsKey("utm_term") ? paramsMap.get("utm_term").get(0) : "";
			String campaign = paramsMap.containsKey("utm_campaign") ? paramsMap.get("utm_campaign").get(0) : "";
			String content = paramsMap.containsKey("utm_content") ? paramsMap.get("utm_content").get(0) : "";
			return Optional.of(new Utm(source, medium, campaign, term, content));
		} else {
	 ...
	 * if not found
	 */
	private ReferrerLookup lookupReferer(String refererHost, String refererPath, Boolean includePath) {

		// Check if domain+full path matches, e.g. for apollo.lv/portal/search/ 
		ReferrerLookup referer = (includePath) ? referers.get(refererHost + refererPath) : referers.get(refererHost);
	 ...
	 */
	private Map<String, ReferrerLookup> loadReferers(InputStream referersYaml) {
		...
		Map<String, ReferrerLookup> referers = new HashMap<>();
		...
		for (Map.Entry<String, Map<String, Map>> mediumReferers : rawReferers.entrySet()) {
			...
			for (Map.Entry<String, Map> referer : mediumReferers.getValue().entrySet()) {
				...
				for (String domain : domains) {
					...
					referers.put(domain, new ReferrerLookup(medium, sourceName));
				}
			}
		}
