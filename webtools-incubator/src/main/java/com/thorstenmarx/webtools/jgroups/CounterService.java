...
/**
 * Provides a consensus based distributed counter (similar to AtomicLong) which
 * can be atomically updated across a cluster.
 *
 * @author Bela Ban
 * @since 0.2
 */
public class CounterService implements StateMachine, RAFT.RoleChange {
	...
	/**
	 * If true, reads can return the local counter value directly. Else, reads
	 * have to go through the leader
	 */
	 ...
	 *
	 * @param name Name of the counter, different counters have to have
	 * different names
	 * @param initial_value The initial value of a new counter if there is no
	 * existing counter. Ignored if the counter already exists
